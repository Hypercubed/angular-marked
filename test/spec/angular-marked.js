...
describe('Directive: marked,', function () {
    ...
    beforeEach(inject(function ($rootScope, $templateCache, _$compile_, _$timeout_) {
    ...
    describe('Element,', function () {
        ...
        it('should convert markdown and compile angular commands', function () {
            $scope.testMessage = '';
            element = $compile('<marked enable-compilation="true">{{ testMessage }}</marked>')($scope);
            $scope.testMessage = '## Element';
            $scope.$digest();
        ...
        it('should compile only once', inject(function ($timeout) {
            
            $scope.testMessage = '';
            element = $compile('<marked enable-compilation="true">{{ testMessage }}</marked>')($scope);

            var directiveScope = $scope.$$childHead;
            spyOn(directiveScope, '$destroy');

            $scope.testMessage = '## Element';
            $scope.$digest();

            $timeout.flush();
            expect(directiveScope.$destroy).toHaveBeenCalled();
        }));
    });
